...
void seeinfo(int a){
    fstream file("file.txt",ios::in);
    fstream get("get.txt",ios ::in);
    marsole ship;
    marsole m;
    string date, date2;
    if (a==1){
        while (!file.eof()){
            file>>ship.postcode>>ship.sendername>>ship.sendercity>>date>>ship.recivername>>ship.recivercity>>ship.postpaid;
            if(!file.eof());
          else break;
            cout<<ship.postcode<<" "<<ship.sendername<<" "<<ship.sendercity<<" "<<date<<" "<<ship.recivername<<" "<<ship.recivercity<<" "<<ship.postpaid<<"\n";
        }
            }
        if(a==2){
            while(!get.eof()){
            get>>m.postcode>>date2;
            cout<<m.postcode<<" "<<date2<<endl;
        }
        }
...
void menu(){
    ...
    cout
            ...
            << "9-Number of intercity shipments(BadWork)\n"
            << "10-adminstrator\n"
            << "11-Info \n\n"
